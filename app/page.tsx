...
import { readStoredTrades, TRADES_STORAGE_KEY, type StoredTrade } from "@/lib/tradesStorage";
          ...
          {orderedTrades.length > 0 ? (
            <ul className="mt-6 space-y-3">
              {orderedTrades.map((trade, index) => (
                <li key={trade.id}>
                  <Link
                    href={`/new-trade?tradeId=${encodeURIComponent(trade.id)}`}
                    className="flex items-center gap-4 rounded-2xl border border-border bg-white/80 px-5 py-4 text-left shadow-sm shadow-black/5 transition hover:-translate-y-0.5 hover:shadow-md"
                  >
