...
export function LibraryCard({
  ...
  const baseVisualWrapperClassName =
    "flex flex-1 w-full items-center justify-center overflow-hidden transition-colors duration-300";
  const resolvedVisualWrapperClassName = visualWrapperClassName
    ? `${baseVisualWrapperClassName} ${visualWrapperClassName}`
    : `${baseVisualWrapperClassName} aspect-[3/4] rounded-md bg-white md:aspect-[4/3]`;
  ...
  return (
    <button
      ...
      className={`group relative flex h-full min-w-[160px] flex-col items-stretch gap-4 rounded-lg border border-[#E6E6E6] bg-[#F7F7F7] p-4 text-center text-xs font-semibold text-fg shadow-[0_20px_50px_-35px_rgba(15,23,42,0.25)] transition-all duration-300 ease-out focus:outline-none focus:ring-0 focus-visible:outline-none focus-visible:ring-0 disabled:pointer-events-none disabled:opacity-50 sm:min-w-[180px] md:h-auto md:items-center transform-gpu will-change-transform ${
        isActive
