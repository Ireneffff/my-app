...
export default function ThemeToggle() {
  ...
  return (
    <button
      ...
      onClick={() => setDark((v) => !v)}
      className="pointer-events-auto inline-flex items-center gap-4 rounded-full border border-border/60 bg-bg px-4 py-2 shadow-lg shadow-black/10 transition hover:shadow-black/20"
    >
      <span className={`h-5 w-5 rounded-full ${dark ? "" : "ring-2 ring-accent"}`}>
        {/* sun */}
